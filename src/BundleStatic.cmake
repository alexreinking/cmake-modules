...
function(cmrk_bundle_static TARGET)
    set(options)
    set(oneValueArgs)
    ...
    while (queue)
        ...
        _cmrk_transfer_same(
            PROPERTIES IMPORTED_COMMON_LANGUAGE_RUNTIME
            FROM ${lib} TO ${objectLib}
        )

        _cmrk_transfer_locations(FROM ${lib} TO ${objectLib})
...
function(_cmrk_transfer_append)
    set(options)
    set(oneValueArgs FROM TO PROPERTIES)
    set(multiValueArgs)
    cmake_parse_arguments(ARG "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
...
function(_cmrk_transfer_locations)
    set(options)
